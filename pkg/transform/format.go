...
func (f *Formatter) Format(n node.Node) node.Node {
	switch typed := n.(type) {
...
func (f *Formatter) formatMapping(m *node.MappingNode) *node.MappingNode {
...
func (f *Formatter) formatSequence(s *node.SequenceNode) *node.SequenceNode {
...
func (f *Formatter) formatScalar(s *node.ScalarNode) *node.ScalarNode {
	s.BaseNode = f.formatBaseNode(s.BaseNode, true)
	return s
}

// formatBaseNode formats comments in a base node
func (f *Formatter) formatBaseNode(base node.BaseNode, isScalar bool) node.BaseNode {
...
func (f *Formatter) getBlankLinesForComment(cg *node.CommentGroup, pos node.CommentPosition) int {
	...
	case node.CommentPositionKey:
		return f.config.BlankLinesBeforeKeyComment
	case node.CommentPositionValue:
...
func (f *Formatter) shouldAddBlankLineBetweenPairs(prev, curr *node.MappingPair) bool {
	...
	if prevValue, ok := prev.Value.(*node.ScalarNode); ok {
...
func FormatWithSorting(n node.Node, sortConfig *SortConfig, formatConfig *FormatConfig) node.Node {
	// First format
