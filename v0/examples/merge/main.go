...
database:
  host: db.example.com`
	...
	result, err := merge2.MergeStrings(baseYAML, overrideYAML)
	if err != nil {
		log.Fatal(err)
	}
...
list: [4, 5]`
	...
	// Deep merge
	deepResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
		merge2.DefaultOptions().WithStrategy(merge2.StrategyDeep))
	fmt.Println("Deep Merge:")
	fmt.Println(deepResult)

	// Shallow merge
	shallowResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
		merge2.DefaultOptions().WithStrategy(merge2.StrategyShallow))
	fmt.Println("\nShallow Merge:")
	fmt.Println(shallowResult)

	// Override strategy
	overrideResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
...
settings:
  ...
  - name: cache
	...
	replaceResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
		merge2.DefaultOptions().WithArrayStrategy(merge2.ArrayReplace))
	fmt.Println("Array Replace:")
	fmt.Println(replaceResult)

	// Append arrays
	appendResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
		merge2.DefaultOptions().WithArrayStrategy(merge2.ArrayAppend))
	fmt.Println("\nArray Append:")
	fmt.Println(appendResult)

	// Merge by index
	indexResult, _ := merge2.MergeStringsWithOptions(baseYAML, overrideYAML,
		merge2.DefaultOptions().WithArrayStrategy(merge2.ArrayMergeByIndex))
	fmt.Println("\nArray Merge by Index:")
	fmt.Println(indexResult)
}

func fileMergeDemo(dir string) {
	// For demo, we'll use the values-with-comments example if it exists
	valuesDir := filepath.Join(filepath.Dir(dir), "values-with-comments")
	valuesFile := filepath.Join(valuesDir, "values-with-comments.yaml")
	valuesMergeFile := filepath.Join(valuesDir, "values-with-comments-merge.yaml")

	// Try to use values-with-comments example files if they exist
	result, err := merge2.MergeFiles(valuesFile, valuesMergeFile)
	if err != nil {
		// Fallback to simple demo
		fmt.Println("Using demo values-with-comments (values-with-comments example not found)")
		baseYAML := `name: app
version: 1.0.0`
		overrideYAML := `version: 2.0.0
env: production`
		result, _ = merge2.MergeStrings(baseYAML, overrideYAML)
	} else {
		fmt.Println("Merged values-with-comments.yaml with values-with-comments-merge.yaml:")
	}
...
final: value3`
	...
	for i := 1; i < len(nodes); i++ {
		merged, err := merge2.MergeStrings(result, nodes[i])
		if err != nil {
			log.Fatal(err)
		}
		result = merged
...
`
	...
	opts := &merge2.Options{
		Strategy:           merge2.StrategyDeep,
		PreserveComments:   false,
		PreserveBlankLines: false,
		ArrayMergeStrategy: merge2.ArrayReplace,
	}

	resultNoComments, err := merge2.MergeStringsWithOptions(baseYAML, overrideYAML, opts)
	if err != nil {
		log.Fatal(err)
	}
