...
`
	...
	sortConfig := &transform2.SortConfig{
		Mode:   transform2.SortModeAscending,
		SortBy: transform2.SortByKey,
	}

	sorter := transform2.NewSorter(sortConfig)
...
`
	...
	programmaticAST := builder.BuildMapping([]*node2.MappingPair{
		{
			Key:   builder.BuildScalar("name", node2.StylePlain),
			Value: builder.BuildScalar("Generated App", node2.StylePlain),
		},
		{
			Key: builder.BuildScalar("config", node2.StylePlain),
			Value: builder.BuildMapping([]*node2.MappingPair{
				{
					Key:   builder.BuildScalar("enabled", node2.StylePlain),
					Value: builder.BuildScalar("true", node2.StylePlain),
				},
				{
					Key:   builder.BuildScalar("level", node2.StylePlain),
					Value: builder.BuildScalar("info", node2.StylePlain),
				},
			}, node2.StyleBlock),
		},
		{
			Key: builder.BuildScalar("modules", node2.StylePlain),
			Value: builder.BuildSequence([]node2.Node{
				builder.BuildScalar("core", node2.StylePlain),
				builder.BuildScalar("auth", node2.StylePlain),
				builder.BuildScalar("api", node2.StylePlain),
			}, node2.StyleBlock),
		},
	}, node2.StyleBlock)

	// Add comments
	node2.AssociateComment(programmaticAST, "# Generated configuration", node2.CommentPositionAbove, 0)
