...
func Merge(base, override node.Node) (node.Node, error) {
	return WithOptions(base, override, DefaultOptions())
}

// WithOptions combines two YAML nodes with the specified options
func WithOptions(base, override node.Node, opts *Options) (node.Node, error) {
	if opts == nil {
		opts = DefaultOptions()
	}
...
// Strings merges two YAML strings and returns the result as a string
func Strings(baseYAML, overrideYAML string) (string, error) {
	return StringsWithOptions(baseYAML, overrideYAML, DefaultOptions())
}

// StringsWithOptions merges two YAML strings with the specified options
func StringsWithOptions(baseYAML, overrideYAML string, opts *Options) (string, error) {
	// Parse base YAML
...
// Files merges two YAML files and returns the result as a string
func Files(basePath, overridePath string) (string, error) {
	return FilesWithOptions(basePath, overridePath, DefaultOptions())
}

// FilesWithOptions merges two YAML files with the specified options
func FilesWithOptions(basePath, overridePath string, opts *Options) (string, error) {
	// Read base file
	...
	// Merge strings
	return StringsWithOptions(string(baseData), string(overrideData), opts)
}

// FilesToFile merges two YAML files and writes the result to a file
func FilesToFile(basePath, overridePath, outputPath string) error {
	return FilesToFileWithOptions(basePath, overridePath, outputPath, DefaultOptions())
}

// FilesToFileWithOptions merges two YAML files and writes the result to a file with options
func FilesToFileWithOptions(basePath, overridePath, outputPath string, opts *Options) error {
	result, err := FilesWithOptions(basePath, overridePath, opts)
	if err != nil {
		return err
	}
...
// Multiple merges multiple YAML nodes in sequence
func Multiple(nodes []node.Node) (node.Node, error) {
	return MultipleWithOptions(nodes, DefaultOptions())
}

// MultipleWithOptions merges multiple YAML nodes in sequence with options
func MultipleWithOptions(nodes []node.Node, opts *Options) (node.Node, error) {
	if len(nodes) == 0 {
	...
	for i := 1; i < len(nodes); i++ {
		merged, err := WithOptions(result, nodes[i], opts)
		if err != nil {
